...
//authenticate
router.get("/", auth, (req, res) => {             //esta ruta se utiliza para que cada vez que yo recargue la pagina
                                                  //con un useffect voy a dispachar esta accion
  User.findByPk(req.user.id).then(user => {      //lo que hace es econtrar el usuario por id, este id proviene del middleware auth
...
//Promote User
router.post("/promote", auth, isAdmin, (req, res) => {
  User.update(
    {
      userRole: "admin",
    },
